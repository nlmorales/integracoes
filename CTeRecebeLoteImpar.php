...
if(controlaExecucao('datacterecebeloteimpar', 'N', 2) == false) {
...
$QueryProtocolo = "SELECT recibo, idemissor, tpamb FROM tb_emicte WHERE numLote % 2 = 1 AND status = 'enviado' AND data_envio > '".date("Y-m-d H:i:s", strtotime("- 15 Days"))."' AND recibo != '' GROUP BY recibo ORDER BY data_emis ASC, prioridade ASC LIMIT 15";
...
while ($RowProtocolo = mysql_fetch_assoc($ResultProtocolo)){
	...
	if($Result['retConsReciCTe']['cStat'] == '104'){
		...
		controlaExecucao('datacterecebeloteimpar', 'S');
		exit();
